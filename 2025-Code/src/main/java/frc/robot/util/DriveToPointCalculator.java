...
public class DriveToPointCalculator {
        public static Pose2d CalculatePointBlue(Pose3d tagPosition, boolean isA) {
                double reefAngle = tagPosition.getRotation().getAngle();
                double perpendicularReefAngle = reefAngle - Math.toRadians(90);
                boolean applyLeftOffset = !isA;

                double leftRightOffsetX = (applyLeftOffset ? offsetOfArmReefLeft : offsetOfArmReef)
                                * Math.cos(perpendicularReefAngle)
                                + (reefWidth / 2) * Math.cos(perpendicularReefAngle + (isA ? 0 : Math.PI));
                double leftRightOffsetY = (applyLeftOffset ? offsetOfArmReefLeft : offsetOfArmReef)
        ...
        public static Pose2d CalculatePointRed(Pose3d tagPosition, boolean isA) {
                double reefAngle = tagPosition.getRotation().getAngle();
                double perpendicularReefAngle = reefAngle - Math.toRadians(90);
                boolean applyLeftOffset = isA;
